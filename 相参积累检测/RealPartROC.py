...
# 门限范围从-2.1到1.8，间隔为0.1
thresholds = np.arange(-4.1, 1.8, 0.1)
# 用于保存每个门限下的虚警率和检测率
far_values = []
pd_values = []
...
for threshold in tqdm(thresholds, desc="Testing thresholds"):
    with torch.no_grad():
        ...
        # y_test_pred= F.softmax(outputs, dim=0)
...
# 绘制 FAR vs Threshold
plt.subplot(1, 2, 1)
plt.plot(thresholds, far_values, label="False Alarm Rate (PRF)", color='blue')
plt.xlabel("Threshold")
plt.ylabel("False Alarm Rate (PRF)")
plt.title("Threshold vs PRF")
plt.grid(True)
